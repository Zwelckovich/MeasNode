// dragdrop.js - Node dragging functionality with loop region integration
 ...
 * Updates loop regions when nodes move.
 *
 * For group dragging:
...
export function makeDraggable($node) {
  $node.on("mousedown", function(ev) {
    ...
    function onMouseMove(ev2) {
      ...
      // Update loop regions after node movement
      if (window.updateLoopRegions) {
        window.updateLoopRegions();
      }
    }
    ...
    $(document).on("mouseup.nodeDrag", function() {
      ...
      // Final update of loop regions when dragging ends
      if (window.updateLoopRegions) {
        window.updateLoopRegions();
      }
    });
